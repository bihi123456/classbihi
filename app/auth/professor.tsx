...
export default function ProfessorAuthScreen() {
  ...
  const validateForm = () => {
    ...
    if (!isLogin) {
      if (!formData.professorNumber.trim()) {
        newErrors.professorNumber = i18n.t('required');
      }

      if (!formData.subject) {
        newErrors.subject = i18n.t('required');
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isLogin) {
        const success = await login(formData.email, formData.password, 'professor');
        if (success) {
        ...
        const success = await register(formData, 'professor');
        if (success) {
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>🧑‍🏫 {i18n.t('professorSpace')}</Text>
        </View>
        ...
        <View style={styles.content}>
          ...
          <View style={styles.form}>
            ...
            {!isLogin && (
              <View style={styles.inputGroup}>
                <Text style={styles.label}>{i18n.t('professorNumber')}</Text>
                <TextInput
                  style={[
                    styles.input,
                    errors.professorNumber && styles.inputError
                  ]}
                  value={formData.professorNumber}
                  onChangeText={(text) => updateFormData('professorNumber', text)}
                  placeholder={i18n.t('professorNumber')}
                  placeholderTextColor={colors.textSecondary}
                  keyboardType="numeric"
                />
                {errors.professorNumber && <Text style={styles.errorText}>{errors.professorNumber}</Text>}
              </View>
            )}
            ...
            {!isLogin && (
              <View style={styles.inputGroup}>
                <Text style={styles.label}>{i18n.t('selectSubject')}</Text>
                <TouchableOpacity
                  style={[
                    styles.input,
                    styles.picker,
                ...
                >
                  <Text style={[
                    ...
                    {formData.subject || i18n.t('selectSubject')}
                  </Text>
                ...
                {showSubjectPicker && (
                  <View style={styles.pickerMenu}>
                    {SUBJECTS.map((subject) => (
                      ...
                      >
                        <Text style={styles.pickerMenuItemText}>{subject}</Text>
                      </TouchableOpacity>
                    ))}
                  </View>
                )}
